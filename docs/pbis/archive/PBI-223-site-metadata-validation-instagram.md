# PBI-223: Site metadata validation contract — Instagram
...
## Problem
Metadata ingestion for `Instagram` is not currently enforced with a strict, testable validation contract. For release, `Instagram` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Instagram)
- shortcode/id
- owner username/id
- caption
- media_type
...
## Required Normalized Metadata Fields (Instagram)
- `remote_post_id`
- `creator`
- `description` (caption)
- `source_urls[]`
...
## Implementation
1. Define `Instagram` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Instagram` payloads.

## Acceptance Criteria
1. `Instagram` payloads missing required keys fail validation deterministically.
2. `Instagram` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Instagram` payload -> `valid=true`, zero missing fields, normalized preview populated.
