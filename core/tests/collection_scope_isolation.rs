...
async fn collection_members_hidden_from_general_scopes() {
    ...
    let standalone = harness
        .insert_test_file("col_standalone", "standalone.png", 1)
        .await;
    let member_a = harness
        .insert_test_file("col_member_a", "member_a.png", 1)
        .await;
    let member_b = harness
        .insert_test_file("col_member_b", "member_b.png", 1)
        .await;
    ...
    assert_eq!(
        col_res.items.len(),
        2,
        "collection scope should show both members"
    );
    ...
    assert!(
        hashes.contains(&"col_standalone"),
