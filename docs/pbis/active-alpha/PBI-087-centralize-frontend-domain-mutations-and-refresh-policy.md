...
1. UI components still execute domain mutations directly and manually fan out sidebar refreshes:
   - `./src/components/sidebar/FolderTree.tsx` (many `FolderController.*` + `SidebarController.requestRefresh()` calls)
   - `./src/components/image-grid/ImageGrid.tsx` (many mutation call-sites with post-mutation refresh chaining)
   - `./src/components/image-grid/SubfolderGrid.tsx:130`
   - `./src/hooks/useInspectorData.ts:344,360`
...
- `./src/components/image-grid/SubfolderGrid.tsx`
- `./src/hooks/useInspectorData.ts`
- new shared mutation layer under `./src/domain/actions/`
