# PBI-210: Site metadata validation contract — Fantia
...
## Problem
Metadata ingestion for `Fantia` is not currently enforced with a strict, testable validation contract. For release, `Fantia` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Fantia)
- `id` (post id)
- creator identity
- `title`
- body/description
...
## Required Normalized Metadata Fields (Fantia)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `Fantia` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Fantia` payloads.

## Acceptance Criteria
1. `Fantia` payloads missing required keys fail validation deterministically.
2. `Fantia` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Fantia` payload -> `valid=true`, zero missing fields, normalized preview populated.
