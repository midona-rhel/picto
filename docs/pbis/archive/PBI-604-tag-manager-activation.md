...
merge, delete, aliases, and implications. The rebuilt frontend exposes a Tag
...
4. Support rename, merge, delete, alias, and implication editing.
...
- Rename, merge, delete, alias, and implication edits persist and settle visibly.
...
Accepted on 2026-08-14 after the Electron smoke confirmed browsing, filtering, and the modal
editing workflow. The backend suite passed with 317 executed tests, the focused frontend suite
passed 10 tests without React update warnings, and TypeScript and command parity were clean.

The current surface is behavior-complete. Final visual direction is intentionally deferred until
the separate UI reference pass can inspect reference application's packaged interface; that work must not reopen
or duplicate the canonical tag behavior implemented here.
