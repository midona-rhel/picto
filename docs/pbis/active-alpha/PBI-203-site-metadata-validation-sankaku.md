# PBI-203: Site metadata validation contract — Sankaku
...
## Problem
Metadata ingestion for `Sankaku` is not currently enforced with a strict, testable validation contract. For release, `Sankaku` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Sankaku)
- `id`
- `tags`/category tags
- `file_url`
- `source`
...
## Required Normalized Metadata Fields (Sankaku)
- `remote_post_id`
- `tags[]`
- `source_urls[]`
- `rating`
...
## Implementation
1. Define `Sankaku` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Sankaku` payloads.

## Acceptance Criteria
1. `Sankaku` payloads missing required keys fail validation deterministically.
2. `Sankaku` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Sankaku` payload -> `valid=true`, zero missing fields, normalized preview populated.
