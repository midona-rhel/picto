...
### Delivered
- `MutationFacts` carries 5 change descriptors (`status_changed`, `tags_changed`, `tag_structure_changed`, `folder_membership_changed`, `view_prefs_changed`) plus `extra_grid_scopes` escape hatch
- `derive_invalidation()` in `runtime_contract/mutation.rs` centralises facts → invalidation mapping
- `MutationImpact` builder is fact-first — legacy `Invalidate` struct removed
- `emit_mutation()` calls `derive_invalidation()` and merges `extra_grid_scopes`
- All ~40 handler call sites updated
- Frontend `deriveStaleResources()` in `resourceInvalidator.ts` derives entirely from `receipt.facts` — no longer reads `receipt.invalidate`
- `DerivedInvalidation` demoted to transitional compatibility status (doc comments + field ordering signal deprecation)
- Orchestration tests updated to fact-based API
...
## Scope
- `./core/src/events.rs`
- `./core/src/runtime_contract/mutation.rs`
- mutation-emitting typed handlers under `./core/src/dispatch/typed/`
