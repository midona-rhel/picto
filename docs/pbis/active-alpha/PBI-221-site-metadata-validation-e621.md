# PBI-221: Site metadata validation contract — e621
...
## Problem
Metadata ingestion for `e621` is not currently enforced with a strict, testable validation contract. For release, `e621` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (e621)
- `id`
- `tags` object by category
- `sources`
- `rating`
- file/media URL
- uploader/artist fields if present

## Required Normalized Metadata Fields (e621)
- `remote_post_id`
- `tags[]` with category namespace mapping
- `creator` tag extraction
- `source_urls[]`
- `rating`
- `post_published_at` (if present)

## Implementation
1. Define `e621` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `e621` payloads.

## Acceptance Criteria
1. `e621` payloads missing required keys fail validation deterministically.
2. `e621` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `e621` payload -> `valid=true`, zero missing fields, normalized preview populated.
