...
pub static SITES: &[SiteEntry] = &[
    ...
    SiteEntry {
        ...
        supports_query: false,
        supports_account: true,
        // The public site exposes its configured content filters without a
        // stored session; a full anonymous certification passes.
        auth_required_for_full_access: false,
        auth_strictly_required: false,
        credential_types: COOKIE_CREDENTIAL_TYPES,
        oauth_provider: None,
    ...
    SiteEntry {
        ...
        name: "Pawchive",
        example_query: "https://pawchive.pw/patreon/user/12345",
        supports_query: false,
        supports_account: true,
        auth_required_for_full_access: false,
        auth_strictly_required: false,
...
pub fn normalize_archive_creator_url(raw: &str, canonical_host: &str) -> Result<String, String> {
    let mut url = Url::parse(raw.trim())
        .map_err(|_| "Archive mirror subscriptions require a concrete creator URL".to_string())?;
    if !matches!(url.scheme(), "http" | "https")
        ...
        || url.host_str() != Some(canonical_host)
    {
        return Err("Archive mirror subscriptions require a canonical creator URL".to_string());
    }
    let segments = url
        .path_segments()
        .into_iter()
        ...
        .collect::<Vec<_>>();
    let [service, "user", creator_id] = segments.as_slice() else {
        return Err("Archive mirror subscriptions require a /service/user/id URL".to_string());
    };
    for value in [*service, *creator_id] {
        if value.is_empty()
            || value.len() > 128
            || !value
                .bytes()
        ...
        {
            return Err("Archive mirror creator identity is invalid".to_string());
        }
    }
    let service = (*service).to_string();
    let creator_id = (*creator_id).to_string();
    url.set_scheme("https")
        .map_err(|_| "Archive mirror URL is invalid".to_string())?;
    url.set_path(&format!("/{service}/user/{creator_id}"));
    Ok(url.to_string())
}
...
fn build_subscribestar_creator_url(creator: &str) -> Option<String> {
    // Start on the canonical public host and let the site's redirect retain
    // the creator slug.
...
mod tests {
    ...
    fn supported_sources_have_coherent_login_contracts() {
        for source in SITES {
            ...
            if source.credential_types.is_empty() {
    ...
    #[test]
    fn baraag_uses_its_managed_mastodon_oauth_contract() {
