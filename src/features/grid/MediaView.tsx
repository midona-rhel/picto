...
import { MediaItem, isVideoMime, toMasonryItem } from './shared';
...
export function MediaView({ images, currentIndex, onNavigate, onClose, onStateChange, onImageChange, totalCount, inboxMode, onInboxAction }: MediaViewProps) {
  ...
  const loadCollectionOutline = useCallback(async (entityId: number) => {
    if (collectionLoadingRef.current) return;
    collectionLoadingRef.current = true;
    try {
      const resp = await api.grid.getOutline({
        limit: null as never,
        cursor: null,
        sortField: 'ordinal',
        sortOrder: 'asc',
      ...
      setCollectionImages(resp.items.map(toMasonryItem));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    setCollectionImages([]);
    void loadCollectionOutline(entityId);
  }, [isCollection, currentImage?.entity_id, loadCollectionOutline]);
