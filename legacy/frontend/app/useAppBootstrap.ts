...
import { folderNodesAtom, smartFoldersAtom } from '../state/sidebar';
...
export function useAppBootstrap(): AppBootstrap {
  ...
  const handleScopeTransitionMidpoint = useCallback(() => {
    ...
    const currentFolderNodes = store.get(folderNodesAtom);
    const currentSmartFolders = store.get(smartFoldersAtom);
    const activeSmartFolder = currentSmartFolders.find((folder) => folder.id === state.activeSmartFolderId);
    setDisplayedTitle(deriveNavigationTitle({
      ...
      activeFolderLabel: state.activeFolderId != null
        ? currentFolderNodes.find((node) => node.id === `folder:${state.activeFolderId}`)?.name ?? null
        : null,
      activeSmartFolderLabel: activeSmartFolder?.name ?? null,
