# PBI-204: Site metadata validation contract — IdolComplex
...
## Problem
Metadata ingestion for `IdolComplex` is not currently enforced with a strict, testable validation contract. For release, `IdolComplex` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (IdolComplex)
- `id`
- `tags`/category tags
- `file_url`
- `source`
...
## Required Normalized Metadata Fields (IdolComplex)
- `remote_post_id`
- `tags[]`
- `source_urls[]`
- `rating`
...
## Implementation
1. Define `IdolComplex` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `IdolComplex` payloads.

## Acceptance Criteria
1. `IdolComplex` payloads missing required keys fail validation deterministically.
2. `IdolComplex` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `IdolComplex` payload -> `valid=true`, zero missing fields, normalized preview populated.
