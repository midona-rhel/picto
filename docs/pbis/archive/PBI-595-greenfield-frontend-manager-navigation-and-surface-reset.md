...
Not implemented. Subscriptions has a dedicated surface, but tags and duplicates still render
the unavailable placeholder. Their functional manager components remain quarantined under
`legacy/frontend/`. PBI-604 owns the rebuilt tag surface; PBI-577 owns duplicate review. This
PBI owns only the shared manager routing/shell and removal of fake scope behavior.
...
The rebuilt shell, API, controller, state, and CSS foundations are live. Their historical plans
