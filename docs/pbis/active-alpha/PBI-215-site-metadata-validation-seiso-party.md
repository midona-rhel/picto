# PBI-215: Site metadata validation contract — Seiso
...
## Problem
Metadata ingestion for `Seiso` is not currently enforced with a strict, testable validation contract. For release, `Seiso` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Seiso)
- `service`
- `user`/creator id
- `id` (post id)
- post title/content
...
## Required Normalized Metadata Fields (Seiso)
- `remote_post_id`
- `remote_creator_id`
- `creator`
- `title`
...
## Implementation
1. Define `Seiso` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Seiso` payloads.

## Acceptance Criteria
1. `Seiso` payloads missing required keys fail validation deterministically.
2. `Seiso` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Seiso` payload -> `valid=true`, zero missing fields, normalized preview populated.
