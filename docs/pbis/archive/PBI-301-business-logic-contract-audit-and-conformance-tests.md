...
### What's done:
1. 13 scope contract conformance tests added to `core/tests/orchestration.rs`.
2. 3 new `TestHarness` helpers: `bitmaps_mark_inbox`, `bitmaps_mark_trash`, `bitmaps_mark_tagged`.
3. Tests directly call `resolve_scope` and `scope_count` from `scope::resolver` — no grid/selection indirection.
4. All PBI-300 business rules are covered: status scopes, untagged, uncategorized, tag intersection/union/exclusion, folder union/intersection/exclusion, grid-selection parity, scope_count agreement.
5. `guard:scope-contract` script (`scripts/guard-scope-contract.mjs`) — scans Rust consumer files for scope business-rule comments that don't cite the canonical contract (`scope::resolver`). Added to `guard:all` and runs in `gate:legacy`.
6. Note: `orchestration.rs` has pre-existing compilation errors from the ongoing backend rearchitecture (59 errors unrelated to scope tests). The new tests compile correctly once those are fixed.
