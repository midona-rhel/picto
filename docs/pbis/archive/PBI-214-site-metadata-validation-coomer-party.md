# PBI-214: Site metadata validation contract — Coomer
...
## Problem
Metadata ingestion for `Coomer` is not currently enforced with a strict, testable validation contract. For release, `Coomer` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Coomer)
- `service`
- `user`/creator id
- `id` (post id)
- post title/content
...
## Required Normalized Metadata Fields (Coomer)
- `remote_post_id`
- `remote_creator_id`
- `creator`
- `title`
...
## Implementation
1. Define `Coomer` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Coomer` payloads.

## Acceptance Criteria
1. `Coomer` payloads missing required keys fail validation deterministically.
2. `Coomer` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Coomer` payload -> `valid=true`, zero missing fields, normalized preview populated.
