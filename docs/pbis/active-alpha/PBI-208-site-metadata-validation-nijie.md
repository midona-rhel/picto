# PBI-208: Site metadata validation contract — Nijie
...
## Problem
Metadata ingestion for `Nijie` is not currently enforced with a strict, testable validation contract. For release, `Nijie` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Nijie)
- `id`
- `user.id`/`user.name`
- `title`
- `description`
...
## Required Normalized Metadata Fields (Nijie)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `Nijie` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Nijie` payloads.

## Acceptance Criteria
1. `Nijie` payloads missing required keys fail validation deterministically.
2. `Nijie` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Nijie` payload -> `valid=true`, zero missing fields, normalized preview populated.
