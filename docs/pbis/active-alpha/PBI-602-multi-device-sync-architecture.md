...
Emission wiring is complete for all truth domains that route through `LibraryDatabase` (2026-07-17): entities (`entity_created` from ingest with a replay-sufficient payload, `entity_status_changed`, `entity_updated`, `entity_deleted`), tags (`entity_tags_added/_removed`, `tag_renamed/_merged/_deleted`, `tag_alias_set`, `tag_implication_set`, `tag_site_mask_set` — masks as decimal strings per PBI-598), folders (create/update/move/delete + `folder_members_added/_removed`), smart folders (create/update/move/delete), collections (create/rename/delete incl. split, `collection_members_added/_removed/_reordered`), and `duplicate_decided` (keyed by the sorted `hashA|hashB` pair, decisions only — detections stay derived). All bulk variants emit the same ops per entity. Op keys: `entity_hash` for entities/collections, `uuid` for folders/smart folders, `namespace:subtag` for tags.
