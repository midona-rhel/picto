# PBI-200: Site metadata validation contract — Danbooru
...
## Problem
Metadata ingestion for `Danbooru` is not currently enforced with a strict, testable validation contract. For release, `Danbooru` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Danbooru)
- `id`
- `tags_artist`/`tags_general`/category tags
- `file_url`
- `source`
- `rating`
- `artist_commentary` (if present)

## Required Normalized Metadata Fields (Danbooru)
- `remote_post_id`
- `creator` tag(s)
- `title` from commentary fallback
- `description` from commentary fallback
- `source_urls[]`
- `tags[]`
- `rating`

## Implementation
1. Define `Danbooru` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Danbooru` payloads.

## Acceptance Criteria
1. `Danbooru` payloads missing required keys fail validation deterministically.
2. `Danbooru` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Danbooru` payload -> `valid=true`, zero missing fields, normalized preview populated.
