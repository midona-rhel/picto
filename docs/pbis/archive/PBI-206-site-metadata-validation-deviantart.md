# PBI-206: Site metadata validation contract — DeviantArt
...
## Problem
Metadata ingestion for `DeviantArt` is not currently enforced with a strict, testable validation contract. For release, `DeviantArt` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (DeviantArt)
- `id` (deviation id)
- `author.username`
- `title`
- `description`
...
## Required Normalized Metadata Fields (DeviantArt)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `DeviantArt` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `DeviantArt` payloads.

## Acceptance Criteria
1. `DeviantArt` payloads missing required keys fail validation deterministically.
2. `DeviantArt` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `DeviantArt` payload -> `valid=true`, zero missing fields, normalized preview populated.
