...
## Completion Notes
1. The original flat-root controller homes were already gone before this slice.
2. The remaining domain `*Controller` aliases were removed in favor of plain domain service modules.
3. `folders/controller.rs`, `tags/controller.rs`, `subscriptions/controller.rs`, and `subscriptions/subscription_group_controller.rs` were deleted.
4. Callers now use domain service modules directly, without zero-state controller wrappers.
