# PBI-202: Site metadata validation contract — ArtStation
...
## Problem
Metadata ingestion for `ArtStation` is not currently enforced with a strict, testable validation contract. For release, `ArtStation` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (ArtStation)
- `id` or project identifier
- `title`
- `description`/body
- `user.username`/creator identity
- `tags`
- media asset URLs

## Required Normalized Metadata Fields (ArtStation)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `ArtStation` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `ArtStation` payloads.

## Acceptance Criteria
1. `ArtStation` payloads missing required keys fail validation deterministically.
2. `ArtStation` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `ArtStation` payload -> `valid=true`, zero missing fields, normalized preview populated.
