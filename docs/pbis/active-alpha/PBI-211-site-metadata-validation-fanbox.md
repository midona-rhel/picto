# PBI-211: Site metadata validation contract — Fanbox
...
## Problem
Metadata ingestion for `Fanbox` is not currently enforced with a strict, testable validation contract. For release, `Fanbox` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Fanbox)
- `id` (post id)
- creator identity
- `title`
...
## Required Normalized Metadata Fields (Fanbox)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `Fanbox` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Fanbox` payloads.

## Acceptance Criteria
1. `Fanbox` payloads missing required keys fail validation deterministically.
2. `Fanbox` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Fanbox` payload -> `valid=true`, zero missing fields, normalized preview populated.
