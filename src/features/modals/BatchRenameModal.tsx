...
export function BatchRenameModal({ open, items, onClose, onRename }: {
  ...
  return <GlassModal open={open} onClose={onClose} title={t("Batch Rename {value0} Items", { value0: items.length })} size="md" footer={<>
    <button className={modalStyles.btn} onClick={onClose} type="button">{t("Cancel")}</button>
    <button data-modal-primary="true" className={`${modalStyles.btn} ${modalStyles.btnPrimary}`} onClick={() => onRename(preview)} disabled={invalid} type="button">{t("Rename")}</button>
  </>}>
    <div className={modalStyles.stack}>
      <div className={styles.tabs} role="tablist" aria-label={t("Rename mode")}>
        {(['format', 'replace'] as const).map((value) => <button key={value} className={`${styles.tab} ${mode === value ? styles.tabActive : ''}`} onClick={() => setMode(value)} role="tab" aria-selected={mode === value} type="button">{value === 'format' ? t("Format") : t("Replace")}</button>)}
      </div>
      {mode === 'format' ? <>
        <div className={modalStyles.field}>
          <label className={modalStyles.fieldLabel}>{t("Name format")}</label>
          <GlassInput value={pattern} onChange={(event) => setPattern(event.target.value)} />
          <span className={modalStyles.helpText}>{t("* keeps the original name. %N, %NN, and %NNN add padded numbering.")}</span>
        </div>
        <div className={modalStyles.field}>
          <label className={modalStyles.fieldLabel}>{t("Start at")}</label>
        ...
        <GlassInput value={pattern} onChange={(event) => setPattern(event.target.value)} placeholder={t("Find")} />
        <GlassInput value={replacement} onChange={(event) => setReplacement(event.target.value)} placeholder={t("Replace with")} />
      </div>}
      <div className={styles.preview} aria-label={t("Rename preview")}>
