...
## Completion Note
Implemented during the typed dispatch/backend naming cleanup. The backend now uses `./core/src/dispatch/typed/media_lifecycle.rs`, `./core/src/dispatch/typed/media_io.rs`, and `./core/src/dispatch/typed/media_metadata.rs`, while IPC command strings remain unchanged. Transitional aliasing in `./core/src/import.rs` was also removed.
