# PBI-220: Site metadata validation contract — Rule34.xxx
...
## Problem
Metadata ingestion for `Rule34.xxx` is not currently enforced with a strict, testable validation contract. For release, `Rule34.xxx` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Rule34.xxx)
- `id`
- `tags`
- `file_url`
- `source`
...
## Required Normalized Metadata Fields (Rule34.xxx)
- `remote_post_id`
- `source_urls[]`
- `tags[]`
- `rating`
...
## Implementation
1. Define `Rule34.xxx` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Rule34.xxx` payloads.

## Acceptance Criteria
1. `Rule34.xxx` payloads missing required keys fail validation deterministically.
2. `Rule34.xxx` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Rule34.xxx` payload -> `valid=true`, zero missing fields, normalized preview populated.
