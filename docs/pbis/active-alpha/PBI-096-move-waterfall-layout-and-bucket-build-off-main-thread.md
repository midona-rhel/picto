...
1. Layout/bucket data is computed in renderer React path:
   - `./src/components/image-grid/CanvasGrid.tsx` (layout + bucket memo usage)
   - `./src/components/image-grid/VirtualGrid.tsx` (`computeLayout` and related helpers)
...
## Scope
- `./src/components/image-grid/VirtualGrid.tsx` (layout helpers extraction)
- `./src/components/image-grid/CanvasGrid.tsx`
- new worker path under `./src/components/image-grid/`
