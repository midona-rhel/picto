# PBI-207: Site metadata validation contract — Patreon
...
## Problem
Metadata ingestion for `Patreon` is not currently enforced with a strict, testable validation contract. For release, `Patreon` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Patreon)
- `id` (post id)
- `creator` identity
- `title`
- `content`
...
## Required Normalized Metadata Fields (Patreon)
- `remote_post_id`
- `creator`
- `title`
- `description`
...
## Implementation
1. Define `Patreon` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Patreon` payloads.

## Acceptance Criteria
1. `Patreon` payloads missing required keys fail validation deterministically.
2. `Patreon` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Patreon` payload -> `valid=true`, zero missing fields, normalized preview populated.
