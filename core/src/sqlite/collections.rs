...
pub(crate) fn sync_collection_aggregate_metadata(
    ...
    super::tags::rebuild_tag_counts(conn)?;
...
mod tests {
    ...
    fn add_collection_members_by_hashes_roundtrip() {
        ...
        let tag_a = crate::sqlite::tags::get_or_create_tag(&conn, "artist", "alice").unwrap();
        let tag_b = crate::sqlite::tags::get_or_create_tag(&conn, "", "landscape").unwrap();
        crate::sqlite::tags::tag_entity(&conn, file_a_id, tag_a, "local").unwrap();
        crate::sqlite::tags::tag_entity(&conn, file_b_id, tag_b, "local").unwrap();

        let folder_id = crate::sqlite::folders::create_folder(
            &conn,
            &crate::sqlite::folders::NewFolder {
                name: "F".to_string(),
                parent_id: None,
                icon: None,
                color: None,
        ...
        .unwrap();
        crate::sqlite::folders::add_entity_to_folder(&conn, folder_id, file_a_id).unwrap();
