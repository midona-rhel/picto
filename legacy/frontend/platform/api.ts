...
//
//  These methods call the canonical engine commands (PBI-568).
//  They use canonical types (EntityViewQuery, EntityTarget, etc.)
//  and are the primary API path for activated frontend slices.
// ═══════════════════════════════════════════════════════════════════

const canonicalApi = {
  /** Query the grid using the canonical EntityViewQuery model. */
  queryEntityView: (query: EntityViewQuery) =>
    invoke<EntityViewPage>('query_entity_view', query as unknown as Record<string, unknown>),

  /** Batch-fetch grid items by entity_hash for targeted reconciliation. */
  getGridItems: (hashes: string[]) =>
    invokeTyped('get_entity_grid_items', { entity_hashes: hashes }) as Promise<CanonicalEntityGridItem[]>,

  /** Get entity details by hash. */
  getEntityDetails: (hash: string) =>
    invokeTyped('get_entity_details', { entity_hash: hash }) as Promise<CanonicalEntityDetails | null>,
};

// ═══════════════════════════════════════════════════════════════════
//  ENTITY COMMANDS (canonical — use EntityTarget)
// ═══════════════════════════════════════════════════════════════════
...
//  FULL API SURFACE
//
//  Canonical engine commands are at api.canonical.*.
//  Entity commands are at api.entity.*.
//  Legacy grid commands (get_grid_page_slim) are at api.grid.* and
//  will be removed once the frontend grid model migrates to
//  api.canonical.queryEntityView.
// ═══════════════════════════════════════════════════════════════════

export const api = {
  /** Canonical engine commands — the primary API path for activated slices. */
  canonical: canonicalApi,

  // ── Legacy grid commands ─────────────────────────────────────────
  // TODO: Replace all consumers with api.canonical.queryEntityView,
  ...
  // ── Legacy aliases ───────────────────────────────────────────────
  // TODO: Remove — all consumers should use api.entity.* directly.
  files: entityApi,
  file: entityApi,
...
// ═══════════════════════════════════════════════════════════════════
//  QUERY / COMMAND API SUBSETS
//
//  queryApi: read-only methods (safe to call from effects/hooks)
//  commandApi: write methods (should go through controllers)
//
//  These use the entity namespace (not files/file aliases).
// ═══════════════════════════════════════════════════════════════════

const entityQueryApi = {
  getDetails: api.entity.getDetails,
  getGridItem: api.entity.getGridItem,
  getGridItems: api.entity.getGridItems,
...
export const queryApi = {
  canonical: {
    queryEntityView: canonicalApi.queryEntityView,
    getGridItems: canonicalApi.getGridItems,
    getEntityDetails: canonicalApi.getEntityDetails,
  },
  // Legacy grid reads — TODO: migrate to queryApi.canonical.*
  grid: {
    getPageSlim: api.grid.getPageSlim,
    getOutline: api.grid.getOutline,
    getEntitiesMetadataBatch: api.grid.getEntitiesMetadataBatch,
  },
  entity: entityQueryApi,
  // Legacy aliases — TODO: remove, use queryApi.entity.*
  files: entityQueryApi,
  file: entityQueryApi,
  selection: {
...
export const commandApi = {
  entity: entityCommandApi,
  // Legacy aliases — TODO: remove, use commandApi.entity.*
  files: entityCommandApi,
  file: entityCommandApi,
  selection: {
