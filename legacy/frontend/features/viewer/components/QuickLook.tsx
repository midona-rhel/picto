 ...
 * Thin wrapper around MediaView: provides the overlay chrome (backdrop, exit button,
 * nav toolbar) while MediaView handles all image rendering, zoom, transitions,
 * collections, and keyboard shortcuts.
 */
import { useState, useEffect, useRef, useCallback } from 'react';
import { IconChevronLeft, IconChevronRight, IconX } from '@tabler/icons-react';
import type { MediaItem } from '../../grid/shared';
import { MediaView, type MediaViewState, type MediaViewControls } from './MediaView';
...
export function QuickLook({ images, currentIndex, onNavigate, onClose, onImageChange, onLoadMore, totalCount }: QuickLookProps) {
  ...
  const [viewState, setViewState] = useState<MediaViewState | null>(null);
  ...
  const handleStateChange = useCallback((state: MediaViewState, controls: MediaViewControls) => {
    setViewState(state);
