# PBI-213: Site metadata validation contract — Kemono
...
## Problem
Metadata ingestion for `Kemono` is not currently enforced with a strict, testable validation contract. For release, `Kemono` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Kemono)
- `service`
- `user`/creator id
- `id` (post id)
- post title/content
...
## Required Normalized Metadata Fields (Kemono)
- `remote_post_id`
- `remote_creator_id`
- `creator`
- `title`
...
## Implementation
1. Define `Kemono` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Kemono` payloads.

## Acceptance Criteria
1. `Kemono` payloads missing required keys fail validation deterministically.
2. `Kemono` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Kemono` payload -> `valid=true`, zero missing fields, normalized preview populated.
