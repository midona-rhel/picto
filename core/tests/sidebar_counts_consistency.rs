...
fn get_count(db: &std::sync::Arc<LibraryDatabase>, scope_key: &str) -> i64 {
    scope_count(db, scope_key).expect("scope count")
}
...
async fn sidebar_counts_stay_consistent_across_state_changes() {
    ...
    // 1. Initial counts
    assert_eq!(get_count(&harness.db, "system:active"), 3);
    assert_eq!(get_count(&harness.db, "system:inbox"), 2);
    assert_eq!(get_count(&harness.db, "system:trash"), 1);

    // 2. Accept inbox file → all increases, inbox decreases
    harness
        .db
        .set_entity_status(&[i1], 1, ExpansionMode::EntityOnly)
        .unwrap();
    assert_eq!(get_count(&harness.db, "system:active"), 4);
    assert_eq!(get_count(&harness.db, "system:inbox"), 1);

    // 3. Tag a file → untagged decreases
    let untagged_before = get_count(&harness.db, "system:untagged");
    ...
    let untagged_after = get_count(&harness.db, "system:untagged");
    assert_eq!(
        untagged_after,
        untagged_before - 1,
    ...
    let uncategorized_before = get_count(&harness.db, "system:uncategorized");
    let folder = harness.db.create_folder("Folder", None, None, None).unwrap();
    harness
        .db
        .add_folder_members(folder, &[a2], ExpansionMode::EntityOnly)
        .unwrap();
    let uncategorized_after = get_count(&harness.db, "system:uncategorized");
    assert_eq!(
        uncategorized_after,
    ...
    let all_before = get_count(&harness.db, "system:active");
    let trash_before = get_count(&harness.db, "system:trash");
    harness
        .db
        .set_entity_status(&[a3], 2, ExpansionMode::EntityOnly)
        .unwrap();
    assert_eq!(get_count(&harness.db, "system:active"), all_before - 1);
    assert_eq!(get_count(&harness.db, "system:trash"), trash_before + 1);
    ...
    for (key, scope_key) in scope_checks {
        let count = get_count(&harness.db, key);
