# PBI-216: Site metadata validation contract — Baraag
...
## Problem
Metadata ingestion for `Baraag` is not currently enforced with a strict, testable validation contract. For release, `Baraag` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Baraag)
- `id` (status id)
...
## Required Normalized Metadata Fields (Baraag)
- `remote_post_id`
- `creator`
- `description`
- `source_urls[]`
...
## Implementation
1. Define `Baraag` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Baraag` payloads.

## Acceptance Criteria
1. `Baraag` payloads missing required keys fail validation deterministically.
2. `Baraag` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Baraag` payload -> `valid=true`, zero missing fields, normalized preview populated.
