...
4. `system:all` is the canonical root library scope.
   - `system:all_files` remains backend read-side migration tolerance only.
5. `random` is treated as ordering over the canonical `system:all` dataset, not a separate dataset.
...
3. Backend migration tolerance for legacy `system:all_files` rows remains intentional.
