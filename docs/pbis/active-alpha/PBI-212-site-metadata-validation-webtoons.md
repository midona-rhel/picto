# PBI-212: Site metadata validation contract — Webtoons
...
## Problem
Metadata ingestion for `Webtoons` is not currently enforced with a strict, testable validation contract. For release, `Webtoons` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Webtoons)
- episode id/post id
- series id/title
- creator name
- episode title
- episode page/image URLs
- episode publish info

## Required Normalized Metadata Fields (Webtoons)
- `remote_post_id`
- `remote_series_id`
- `creator`
- `title`
...
## Implementation
1. Define `Webtoons` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Webtoons` payloads.

## Acceptance Criteria
1. `Webtoons` payloads missing required keys fail validation deterministically.
2. `Webtoons` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Webtoons` payload -> `valid=true`, zero missing fields, normalized preview populated.
