# PBI-219: Site metadata validation contract — Yande.re
...
## Required Raw Metadata Keys (Yande.re)
- `id`
- category tag arrays / `tags`
- `file_url`
- `source`
- `rating`

## Required Normalized Metadata Fields (Yande.re)
- `remote_post_id`
- `source_urls[]`
- `tags[]`
- `rating`
...
## Implementation
1. Define `Yande.re` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Yande.re` payloads.

## Acceptance Criteria
1. `Yande.re` payloads missing required keys fail validation deterministically.
2. `Yande.re` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Yande.re` payload -> `valid=true`, zero missing fields, normalized preview populated.
