# PBI-209: Site metadata validation contract — Tumblr
...
## Problem
Metadata ingestion for `Tumblr` is not currently enforced with a strict, testable validation contract. For release, `Tumblr` must fail fast on invalid/missing metadata and expose deterministic API validation diagnostics before subscription runs.
...
## Required Raw Metadata Keys (Tumblr)
- `id` (post id)
- `blog_name`/author
- `summary`/caption/body
- `tags`
- media URLs
- `post_url`

## Required Normalized Metadata Fields (Tumblr)
- `remote_post_id`
- `creator`
- `title` (if present)
- `description`
...
## Implementation
1. Define `Tumblr` schema in a site-metadata schema registry (single source of truth).
...
5. Add fixture-driven tests for both valid and invalid `Tumblr` payloads.

## Acceptance Criteria
1. `Tumblr` payloads missing required keys fail validation deterministically.
2. `Tumblr` normalized metadata is complete and typed per schema.
...
## Test Cases
1. Valid `Tumblr` payload -> `valid=true`, zero missing fields, normalized preview populated.
